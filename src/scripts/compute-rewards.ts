/**
 * Retroactive XP + badge computation.
 * Reads all historical on-chain data, applies DAO-configured action weights,
 * daily caps, and multipliers, then writes UserScore + PointEvent + UserBadge.
 *
 * Safe to re-run: truncates PointEvent and UserScore before rebuilding.
 *
 * Usage: bun run src/scripts/compute-rewards.ts [--dry-run]
 *
 * Flags:
 *   --dry-run   Print computed scores without writing to DB
 *   --no-badges Skip badge computation (faster for XP-only passes)
 */
...
const skipBadges = process.argv.includes("--no-badges");
...
// Sorted ascending by level — used to binary-search the current level
...
function toDate(ts: Date | BigInt | number | string | null): string {
  if (!ts) return "1970-01-01";
  const d = ts instanceof Date ? ts : new Date(Number(ts) * 1000);
  return d.toISOString().slice(0, 10);
}

function levelForXp(xp: number, levels: LevelConfig[]): number {
  let level = 1;
...
function applyCaps(rawEvents: RawEvent[], actionMap: Map<string, ActionConfig>): RawEvent[] {
...
async function main() {
  console.log(`Computing rewards${isDryRun ? " (DRY RUN)" : ""}…`);

  const { actionMap, multipliers, levels } = await loadConfig();
  console.log(`  Config: ${actionMap.size} actions, ${multipliers.length} multipliers, ${levels.length} levels`);

  const first100 = await firstNUsers(100);
  console.log(`  First 100 users identified: ${first100.size} found`);
  ...
  const gather = async (type: string, fn: () => Promise<RawEvent[]>) => {
    ...
    allRaw.push(...events);
    console.log(`  Gathered ${events.length} ${type} events`);
  };
  ...
  if (actionMap.has("offer_accepted_seller") && actionMap.has("offer_accepted_buyer")) {
    ...
    allRaw.push(...events);
    console.log(`  Gathered ${events.length} offer_accepted events`);
  }
  ...
  console.log(`  Total raw events: ${allRaw.length}`);

  // ── Apply daily caps ───────────────────────────────────────────────────────
  const cappedEvents = applyCaps(allRaw, actionMap);
  console.log(`  After daily caps: ${cappedEvents.length} events`);
  ...
  console.log(`  Unique addresses: ${scoresByAddress.size}`);

  // ── Compute badges ─────────────────────────────────────────────────────────
  let badgesByAddress = new Map<string, string[]>();
  if (!skipBadges) {
    badgesByAddress = await computeBadges(scoresByAddress, first100);
    console.log(`  Badge grants computed: ${[...badgesByAddress.values()].reduce((s, b) => s + b.length, 0)}`);
  }
  ...
  if (isDryRun) {
    const top10 = [...scoresByAddress.entries()]
      ...
      .slice(0, 10);
    console.log("\nTop 10 scores:");
    for (const [address, score] of top10) {
      const level = levelForXp(score.totalXp, levels);
      console.log(`  ${address.slice(0, 10)}…  ${score.totalXp} XP  level ${level}`);
    }
  ...
  console.log("  Cleared existing scores/badges/events");
  ...
  console.log(`  Wrote ${scoreCount} UserScore records`);
  ...
  console.log(`  Wrote ${pointEvents.length} PointEvent records`);
