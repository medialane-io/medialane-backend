...
import { rpcEndpoints, rpcFetch } from "../../utils/rpcFetch.js";
...
export default function rpc(fetchImpl: typeof fetch = fetch): Hono<AppEnv> {
  ...
  const upstream = fetchImpl === fetch ? rpcFetch() : fetchImpl;
  ...
  app.post("/", async (c) => {
    ...
    try {
      const res = await upstream(rpcEndpoints()[0]!, {
        method: "POST",
      ...
      log.warn({ method, err }, "RPC upstream unreachable");
      return c.json(rpcError(-32603, "Upstream RPC unreachable"));
    }
  });
